...
def main(args):
    ...
    # TODO
    test_pose_save_dir = os.path.join(root_dir, args.test_pose_save_dir)
    test_video_dir = os.path.join(root_dir, args.test_video_dir) # 'video/test_full_ske'
    ...
    for i in range(0, len(df)):
        ...
        if not os.path.exists(test_pose_save_path): continue
        ...
        for index in index2action:
            ...
            if mae < best_mae:
                best_mae = mae
                best_obo = obo
                final_pose_output = pose_count
        print(f'ground_truth: {gt_count}, pred_count: {final_pose_output}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pth_dir', type=str, metavar='DIR',
                        help='path to a saved weight pth file')
    parser.add_argument('--test_pose_save_dir', type=str,
                        help='the test pose save dir, 5 average angles')
    parser.add_argument('--test_video_dir', type=str,
                        help='the test video dir, video/test or video/test_full_ske')
