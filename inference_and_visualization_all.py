...
class PoseClassificationVisualizer(object):
    ...
    def __call__(self,
        ...
        # this like will paste the density on the frame
    ...
    def _plot_classification_history(self, output_width, output_height):
        ...
        plt.xlabel('Frame')
        plt.ylabel('Density')
        plt.title('RepCount of `{}`'.format(self._class_name))
...
def main(args):
    one_video_name = 'my_full_5_average_angles'
    save_frame_c = False
    save_skeleton_c = False
    save_density_c = True
    density_save_path = 'test_density_img_5_ave_angles'
    save_video_c = True
    ...
    # output video dir
    output_video_dir = os.path.join(root_dir, 'video_visual_output', 'test_' + one_video_name)
    # this includes all test data, the one_video_name is for specific class
    input_video_dir = os.path.join(root_dir, 'video', 'test')  # test
    # the extracted test poses
    poses_save_dir = os.path.join(root_dir, 'test_poses_5_ave')

    if not os.path.isdir(output_video_dir):
        os.makedirs(output_video_dir)

    # the line on the top is for specific class with one_video_name
    ...
    weight_path = 'best_5_average.pth'
    ...
    for i in range(0, len(test_df)):
        ...
        all_landmarks = np.load(poses_save_path).reshape(-1, 104) # 33*3 coordinates +5 joint angles
        ...
        for index in index2action:
            ...
            if mae < best_mae:
                ...
                real_index = index
        # obtain the one with the better performance
        ...
        for output in all_output:
            ...
            # save each RGB frame
            if save_frame_c:
                frames_path = os.path.join(output_video_dir, 'frames')
                if not os.path.exists(frames_path):
                    os.makedirs(frames_path)
                cv2.imwrite(frames_path + '/' + str(frame_count) + '.jpg', input_frame)
            ...
            output_sk_black_frame = np.zeros_like(input_frame)
            ...
            # save the skeleton image on frame background
            if save_skeleton_c:
                skeleton_path = os.path.join(output_video_dir, 'frames_with_skeleton')
                if not os.path.exists(skeleton_path):
                    os.makedirs(skeleton_path)
                cv2.imwrite(skeleton_path + '/' + str(frame_count) + 'fs.jpg', cv2.cvtColor(np.array(output_frame), cv2.COLOR_RGB2BGR))

            if pose_landmarks is not None:
                mp_drawing.draw_landmarks(
                    image=output_sk_black_frame,
                    landmark_list=pose_landmarks,
                    connections=mp_pose.POSE_CONNECTIONS,
                    landmark_drawing_spec=landmark_drawing_spec,
                    connection_drawing_spec=connection_drawing_spec)

            # save the skeleton image on black background
            if save_skeleton_c:
                bl_skeleton_path = os.path.join(output_video_dir, 'black_with_skeleton')
                if not os.path.exists(bl_skeleton_path):
                    os.makedirs(bl_skeleton_path)
                cv2.imwrite(bl_skeleton_path + '/' + str(frame_count) + 'bs.jpg', cv2.cvtColor(np.array(output_sk_black_frame), cv2.COLOR_RGB2BGR))
            ...
            if salient1_triggered:
                ...
                curr_pose = 'salient2'
            # print(pose_count)
            density, output_frame, y_density_list = pose_classification_visualizer(
                frame=output_frame,
                pose_classification=classify_prob,
                pose_classification_filtered=classify_prob,
                repetitions_count=pose_count)

            # # save the density map image
            if save_density_c and frame_count == int(video_n_frames):

                y_density_list = ['test'] + [action_type] + [video_name] + [pose_count] + y_density_list
                # print(y_density_list)
                # Save y values to CSV file
                with open('classification_history_angles.csv', 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(test_df.loc[i])
                    writer.writerow(y_density_list)

                density_path = os.path.join(root_dir, 'video_visual_output', density_save_path)# , video_name.replace('.mp4', '')
                # density_path = os.path.join(output_video_dir, '64f_density_after_compress')
                if not os.path.exists(density_path):
                    os.makedirs(density_path)
                density.savefig(density_path+'/' + video_name.replace('.mp4', '') + 'fs.jpg', dpi=300, bbox_inches='tight')

            # save video with density map simultaneously
            if save_video_c:
